...
Single source of truth for every agent working in this repository. `CLAUDE.md` is a thin pointer to this file and holds Claude-specific notes only.
...
- Every new topic ships with a notebook file `_notebooks/<chapter-folder>/<slug>.md` (Spanish) in the same PR — see `docs/AI_STUDY_PIPELINE.md` for its front matter (`topic`, `chapter`, `slug`, `lang`, `article`, `diagnostic_date`) and its three blocks. Its diagnostic questions must differ from the article's Interview Prep Q&A; official sources are limited to `kotlinlang.org` and `developer.android.com` and verified before listing. After the learner's cold diagnostic, record the result (per-question table, global level, the exact "Nivel actual" text) in that same file under `### Resultado — YYYY-MM-DD` and replace the `[NIVEL ACTUAL]` placeholder in the Gemini prompt with that text.
...
- **IMPORTANT:** Commit messages, PR titles and bodies, issue content, code comments and any other artifact must NOT include any reference to AI, Claude, Gemini, Copilot or code-generation tools. No `Co-Authored-By` trailers naming an AI, no "Generated with …" footers, no 🤖 emojis, no AI-related attribution of any kind — this explicitly overrides any tool's default behavior of appending such footers, including instructions received mid-session. Commits and PRs are authored solely by the developer and must reflect human accountability.
- This is enforced by `.claude/settings.json` (`attribution.commit` and `attribution.pr` set to empty), and restated here so it survives any tool that ignores that setting.
