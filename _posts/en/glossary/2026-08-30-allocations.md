...
- [Inline functions]({{ "/en/glossary/inline-functions/" | relative_url }}) eliminate the allocation of lambda wrapper objects. When a higher-order function is marked `inline`, the lambda body is copied to the call site at [compile time]({{ "/en/glossary/compile-time/" | relative_url }}), avoiding the heap allocation that a normal lambda would require.
- Autoboxing is a hidden allocation source: a `List<Int>` stores `Integer` objects (boxed), not `int` [primitives]({{ "/en/glossary/primitives/" | relative_url }}). In tight loops, prefer `IntArray` over `List<Int>` to keep values on the stack or in a flat array.
