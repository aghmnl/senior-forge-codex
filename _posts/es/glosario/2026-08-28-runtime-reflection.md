...
- **R8/ProGuard y la reflexión entran en conflicto**: R8 renombra y elimina clases/métodos que parecen no usarse. Si tu código los accede solo via reflexión, R8 no ve la referencia y los elimina. Por eso las bibliotecas que usan reflexión (Gson, Retrofit con converters) requieren reglas keep de ProGuard.
- Preferir alternativas de [tiempo de compilación]({{ "/es/glosario/compile-time/" | relative_url }}) sobre reflexión siempre que sea posible: [procesamiento de anotaciones]({{ "/es/glosario/annotation-processing/" | relative_url }}) (Dagger/Hilt genera código en lugar de reflejar), jerarquías de sealed classes (`when` exhaustivo en lugar de dispatch reflectivo), y parámetros de tipo [`reified`]({{ "/es/glosario/reified/" | relative_url }}) (sustitución de tipos en compile-time en lugar de checks de tipo en runtime).
- `::class` en Kotlin te da una referencia `KClass`. `::class.java` te da el objeto Java `Class`. Ambos son puntos de entrada a reflexión en runtime. Usos comunes: `Room.inMemoryDatabaseBuilder(ctx, MyDb::class.java)`, `startActivity(Intent(this, TargetActivity::class.java))`, frameworks de serialización.
...
`::class.java` es un punto de entrada de reflexión liviano — Room usa el token `Class` para localizar la clase `_Impl` generada por [procesamiento de anotaciones]({{ "/es/glosario/annotation-processing/" | relative_url }}), no para escanear reflectivamente la clase de base de datos.
