...
Cuando una propiedad se accede exclusivamente desde el hilo de UI, [`LazyThreadSafetyMode.NONE`]({{ "/es/glosario/lazy-thread-safety-mode/" | relative_url }}) evita el [bloque `synchronized`]({{ "/es/glosario/synchronized-block/" | relative_url }}) y elimina el overhead de la inicialización thread-safe.
